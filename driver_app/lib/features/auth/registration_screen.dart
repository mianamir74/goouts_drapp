...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  ///   3. Show a bottom sheet of addresses for the user to pick
  ...
  ///
  /// For Northern Ireland (BT) postcodes or new builds where OS has no data,
  /// fall back to a single Mapbox lookup.
  Future<void> _confirmPostcode() async {
    ...
    try {
      ...
      if (osResults.isEmpty) {
        ...
        if (!isNI) {
          // Non-NI postcode with no OS data — show clear error so user can
          // either re-type or use the "Enter manually" link.
          setState(() {
        ...
        final String? matchedCity =
            mapbox.city.isNotEmpty ? _matchCityOption(mapbox.city, cityOptions) : null;
        ...
        setState(() {
          ...
          _addressFieldsLocked = false; // user types address manually
        });

        _showSnackBarMessage(
          'Postcode verified (Northern Ireland). Please type your house no and street name.',
        );
        return;
      }

      // ── OS path: show bottom sheet for the user to pick ─────────────
      setState(() {
        _isConfirmingPostcode = false;
      });
      ...
      if (selected.thoroughfareName.isNotEmpty) {
        _streetNameController.text = selected.thoroughfareName;
      }
      if (selected.resolvedBuildingRef.isNotEmpty) {
        _houseNoController.text = selected.resolvedBuildingRef;
      }
  ...
  /// Returns the chosen address, or null if dismissed / "not in list".
  ...
  /// Builds UPRN-trust UX. Auto-dismisses after ~1.2 seconds.
