...
class AddressLookupService {
  ...
  Future<MapboxAddressResult?> validatePostcode(String postcode) async {
    ...
    if (_mapboxToken.isEmpty) {
      ...
      // No token compiled in. Say so once, loudly. Sending Mapbox an empty
      // access_token returns 401, which every caller here turns into "no
      // addresses found" — indistinguishable from a genuinely unknown
      // postcode, and it would send someone hunting a Mapbox account problem
      // that does not exist.
      if (_mapboxToken.isEmpty) {
        developer.log(
          'MAPBOX_TOKEN is empty. Build with '
          '--dart-define=MAPBOX_TOKEN=<token>. Address lookup is disabled.',
          name: 'AddressLookupService',
          level: 1000,
        );
        return null;
