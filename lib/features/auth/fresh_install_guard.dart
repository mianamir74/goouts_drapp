...
// hands on a phone, the next owner installs GoOuts Driver, and they are signed
...
// is the deliberate choice: a storage error must not log out every driver on
...
Future<void> enforceFreshInstallSignOut() async {
  try {
    ...
    //
    // a SYNCHRONOUS read, and on iOS it is unreliable at this point in
    // startup. Firebase restores the Keychain session ASYNCHRONOUSLY after
    // initializeApp returns. Called immediately, currentUser can still be null
    // while a perfectly good session is milliseconds from arriving.
    //
    // The guard would see nothing, sign nobody out, write its marker and
    // return. Moments later Firebase finishes restoring and the app opens
    // straight into the PREVIOUS OWNER's account — the exact thing this file
    // exists to prevent, failing silently and only on slower or colder
    // devices, which is why testing never caught it.
    //
    // authStateChanges() always emits once with the resolved state, so
    // awaiting the first event asks the question at the right time.
    //
    // The timeout matters: if Firebase never emits — no network on a cold
    // start, an SDK fault — this must not hang before runApp and leave a black
    // screen. Five seconds, then fall back to the synchronous read, which is
    // no worse than the behaviour being replaced.
