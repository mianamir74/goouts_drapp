...
class _HelpSupportScreenState extends State<HelpSupportScreen> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      // Show success dialog with View Ticket button
      await showDialog<void>(
        ...
        builder: (BuildContext dialogContext) {
          return AlertDialog(
            ...
            content: Text(
              'Thanks for contacting us. We are looking into this and someone will be in touch shortly.\n\n'
              'Your ticket number is: $shortTicket\n\n'
              'You can continue this conversation from "My Support Tickets" in Help & Support.',
            ),
            actions: [
              TextButton(
              ...
              child: Form(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _SectionCard(
                      title: 'Your Details',
                      subtitle: 'We use this to identify your account.',
                      children: [
                        TextFormField(
                        ...
                        TextFormField(
                          ...
                          validator: (v) => _requiredValidator(v, 'Surname')),
                        const SizedBox(height: 12),
                        TextFormField(
                        ...
                        TextFormField(
                          ...
                          decoration: _inputDecoration(label: 'Mobile Number'),
                          validator: (v) => _requiredValidator(v, 'Mobile Number')),
                        const SizedBox(height: 12),
                        TextFormField(
                          controller: _referralCodeController,
                          decoration: _inputDecoration(label: 'Your Referral Code'),
                          validator: (v) => _requiredValidator(v, 'Your Referral Code')),
                      ],
                    ),
                    const SizedBox(height: 16),

                    // ── Category + Sub-topic dropdowns ──────────────────────
                    _SectionCard(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<_Category>(
                          ...
                          decoration: _inputDecoration(label: 'Select category'),
                          isExpanded: true,
                          items: _categories.map((cat) => DropdownMenuItem(
                            value: cat,
                            child: Row(children: [
                              Icon(cat.icon, size: 16, color: _goOutsBlue),
                              const SizedBox(width: 10),
                        ...
                        // Sub-topic dropdown — appears after category selected
                        if (_selectedCategory != null &&
                            _selectedCategory!.subTopics.isNotEmpty) ...[
                          const SizedBox(height: 14),
                          DropdownButtonFormField<String>(
                            value: _selectedSubTopic,
                            decoration: _inputDecoration(label: 'Select specific issue'),
                            isExpanded: true,
                            items: _selectedCategory!.subTopics
                                .map((topic) => DropdownMenuItem(
                                  value: topic,
                                  child: Text(topic, style: const TextStyle(
                                    fontSize: 14, color: Color(0xFF1C1C1C))),
                                )).toList(),
                            onChanged: (topic) {
                              setState(() {
                                _selectedSubTopic  = topic;
                                _showCustomSubject = topic == 'Something else';
                            ...
                            validator: (_) =>
                                (_selectedCategory!.subTopics.isNotEmpty &&
                                    _selectedSubTopic == null)
                                    ? 'Please select a specific issue' : null,
                          ),
                        ],

                        // Custom subject text field — shown when Something else selected
                        if (_showCustomSubject) ...[
                          const SizedBox(height: 14),
                          TextFormField(
                            controller: _customSubjectController,
                            decoration: _inputDecoration(label: 'Describe your issue briefly'),
                            validator: (v) => (_showCustomSubject && (v == null || v.trim().isEmpty))
                    ...
                    // ── Message ─────────────────────────────────────────────
                    if (_selectedCategory != null &&
                        (_selectedSubTopic != null || _selectedCategory!.id == 'other')) ...[
                      _SectionCard(
                        title: 'Your Message',
